...
if [[ $EUID -ne 0 ]]; then
	...
	options=(
		...
		3  "Git" off
		4  "Open JDK" off
		5  "IntelliJ IDEA Community 2020.1.2" off
	...
	do
		case $choice in
			...
			7)	
				...
				wget https://download-cf.jetbrains.com/python/pycharm-community-2020.1.2.tar.gz -q --show-progress
				warn "Download complete, extracting..."
				mkdir IDEs 2> /dev/null
				tar -zxf pycharm-community-2020.1.2.tar.gz --directory IDEs
				chown -R $USERNAME IDEs
				success "✓ PyCharm Community 2020.1.2 can be found in `pwd`/IDEs\n"
				;;
			8)
